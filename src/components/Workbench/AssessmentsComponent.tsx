...
import { 
  ...
  Paper, Chip, Card, CardContent, Grid, Divider, Alert, Badge, Tooltip,
  useTheme
} from '@mui/material';
import { 
  Warning, CheckCircle, Info, Security, Assessment, AssignmentLate,
  ReportProblem, Spa, Gavel, HealthAndSafety
...
const AssessmentsComponent: React.FC = () => {
  ...
  const carrierRuleDecisions = selectedCase.assessment.ruleDecisions?.carrier || [];
  const alitheiaAssessments = selectedCase.assessment.ruleDecisions?.alitheia || [];
  
  const referralAssessments = alitheiaAssessments.filter(a => 
    a.refer?.toLowerCase() === 'yes'
  );
  
  const carrierReferrals = carrierRuleDecisions.filter(r => 
    r.refer?.toLowerCase() === 'yes'
  );

  const hasReferrals = referralAssessments.length > 0 || carrierReferrals.length > 0;
  
  // Get risk class summary
  const bestRiskClass = alitheiaAssessments
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={6} md={4}>
              <Box sx={{ 
                ...
                >
                  <Gavel sx={{ fontSize: 32, color: theme.palette.primary.main, mb: 1 }} />
                </Badge>
                <Typography variant="body2">
                  {carrierRuleDecisions.filter(r => r.refer === 'Yes').length > 0 ? 
                    '1 referral needed' : 'No referrals'}
                </Typography>
              </Box>
            </Grid>
            ...
            <Grid item xs={6} md={4}>
              <Box sx={{ 
                ...
                >
                  <Assessment sx={{ fontSize: 32, color: theme.palette.primary.main, mb: 1 }} />
                </Badge>
                <Typography variant="body2">
                  {referralAssessments.length > 0 ? 
                    `${referralAssessments.length} referrals needed` : 'No referrals'}
                </Typography>
              </Box>
            </Grid>
          </Grid>
      ...
      <Card sx={{ 
        ...
        <CardContent>
          ...
          <TableContainer component={Paper} elevation={0} sx={{ 
            ...
            <Table sx={{ minWidth: 650 }} aria-label="carrier rule decisions table" size="small">
              <TableHead>
                <TableRow sx={{ bgcolor: '#f7f9fc' }}>
                  <TableCell sx={{ fontWeight: 600 }}>Rule</TableCell>
                  <TableCell sx={{ fontWeight: 600 }}>Risk Class</TableCell>
                  <TableCell sx={{ fontWeight: 600 }}>Mortality Rating</TableCell>
                  ...
                  <TableCell sx={{ fontWeight: 600 }}>Flat Extra Rating</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                  ...
                  carrierRuleDecisions.map((row, index) => (
                    <TableRow 
                      ...
                      sx={{
                        ...
                        backgroundColor: row.refer?.toLowerCase().includes('yes') 
                    ...
                    >
                      ...
                      <TableCell><StatusChip status={row.refer} /></TableCell>
                      <TableCell><StatusChip status={row.decline} /></TableCell>
                      <TableCell><StatusChip status={row.tobacco} /></TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} align="center" sx={{ py: 3 }}>
                      <Box sx={{ color: 'text.secondary', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <Typography>No carrier rule decisions available</Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                )}
      ...
      <Card sx={{ 
        ...
        <CardContent>
          ...
          <TableContainer component={Paper} elevation={0} sx={{ 
            ...
            <Table sx={{ minWidth: 650 }} aria-label="alitheia assessments table" size="small">
              <TableHead>
                <TableRow sx={{ bgcolor: '#f7f9fc' }}>
                  <TableCell sx={{ fontWeight: 600 }}>Rule</TableCell>
                  <TableCell sx={{ fontWeight: 600 }}>Risk Class</TableCell>
                ...
                {alitheiaAssessments.length > 0 ? (
                  alitheiaAssessments.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {row.overrideReason ? (
                          <Tooltip title={`Reason: ${row.overrideReason}${row.overrideComment ? `\nComment: ${row.overrideComment}` : ''}`}>
                            <Chip 
                              label="View Details" 
                              size="small"
                              color="info"
                  ...
                  <TableRow>
                    <TableCell colSpan={8} align="center" sx={{ py: 3 }}>
                      <Box sx={{ color: 'text.secondary', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <Info sx={{ mb: 1, fontSize: 36, opacity: 0.6 }} />
                        <Typography>No alitheia assessments available</Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                )}
...
const StatusChip: React.FC<{ status: string }> = ({ status }) => {
  ...
  if (status.toLowerCase() === "yes" || status.toLowerCase() === "yes - override") {
    return (
      <Chip 
        label={status}
        size="small"
